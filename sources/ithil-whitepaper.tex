...
Ithil's treasury is made exclusively by ITHIL tokens. This treasury is used to feed the TOL of a given Vault (for bootstrap and boosts, see \ref{glSub}), to pay for community mining initiatives and airdrops (see \ref{acmSub}), and to participate to eventual external investments, managed by Ithil's governance in the form of DAO (see \ref{daoSub}).

\subsection{Airdrops and community mining}\label{acmSub}

At launch, early users are rewarded with NFTs, stablecoins and a small quantity of ITHIL taken from the treasury. Afterwards, active contributors such as developers, marketers or others, can get their share of ITHIL tokens as a reward. 

\subsection{Ithil's DAO}\label{daoSub}

Ithil's DAO is the legal framework for Ithil's treasury and team to operate. Everybody holding a ITHIL token is considered member of the DAO, without any distinction. Notably, the core team is not more entitled, nor more liable, than anybody else on governance decisions.

Anyone holding a ITHIL token can participate in the DAO by voting. The voting system is the one common to all DAO's: a proposal can be launched, and voted on-chain by ITHIL holders. If it reaches enough votes, it is implemented.
