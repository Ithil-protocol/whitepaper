...
\begin{titlepage}
    \begin{center}
        ...
        \begin{abstract}        
        Ithil aims to bring decentralised services to the web3 space via a well-thought system of undercollateralized loans - a game-changer for traders, liquidity providers, and other protocols who can now rely on various capital-efficient products to address their needs. 
...
\item The position is part of the Service's state and it is visible on-chain as a NFT. It can be traded by the user or by Ithil (see {\ref{securitisationSec}). The \verb|quote| public function of the Service allows checking the value of the held tokens with respect to the owed tokens. If a position's value decreases below a specific {\bf risk factor} of the margin, the position is {\bf liquidable} and can be closed by liquidators (see \ref{liqSub0}).
